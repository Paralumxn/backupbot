...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        const addEXP = args[1]
        const user = await Levels.appendXp(target.id, message.guild.id, addEXP)
        const targetEXP = Levels.xpFor(parseInt(user.level) + 1)

        if(!target) return message.reply("Who are you going to add some level?")
        if(!addEXP) return message.reply("How many levels are you going to add?")
