...
module.exports = {
    name: "subtract-exp",
    aliases: ["subtractexp", "subtractxp", "subtract-xp"],
    ...
    async execute(client, message, args) {
        ...
        const subEXP = args[1]
        const user = await Levels.subtractXp(target.id, message.guild.id, subEXP)
        const targetEXP = Levels.xpFor(parseInt(user.level) + 1)

        if(!target) return message.reply("Who are you going to add some level?")
        if(!subEXP) return message.reply("How many levels are you going to add?")
